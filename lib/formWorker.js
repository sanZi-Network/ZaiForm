...
function createForm(title, description, action, userID) {
    const fs = require('fs');
...
function updateForm(formID, userID, data) {
    ...
    if (!data.title && !data.action && !data.fields && !data.enabled && !data.description) return false;

    const formFile = readFormFile(formID, userID);
    if (!formFile) return false;

    formFile.title = data.title || formFile.title;
    formFile.description = data.description || formFile.description;
    ...
    formFile.customize = data.customize || formFile.customize;
    formFile.updateTimestamp = new Date().getTime();
