...
function updateAccountProfile(userID, userName, userEmail, userPass) {
    const crypto = require('crypto');
    const { readTemplate, replaceHTML, sendMail } = require('./lib');
    ...
    if (userEmail && userEmail !== userFile.email) {
        userFile.email = userEmail;
        userFile.mailVerified.status = false;
        userFile.mailVerified.verifyKey = crypto.randomBytes(32).toString('hex');
...
function loginAccount(userName, userPass, isSocial = false) {
    ...
    if (!userFile) return false;
    if (!userFile.enabled) return false;
